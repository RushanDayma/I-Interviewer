const SessionCard = ({ session, onClick, onDelete }) => {
    ...
    const scoreColor = session.status === 'completed' ? (session.overallScore > 75 ? ' text-emerald-500' : 'text-amber-500') : 'text-slate-300';

    return (
        <div onClick={() => onClick(session)} className='group bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 transition-all hover:shadow-xl hover:shadow-indigo-900/5 hover:border-indigo-100 active:scale-[0.98] cursor-pointer'>
            <div className='flex items-center gap-4 sm:gap-6 w-full md:w-auto flex-grow'>
                ...
                <div className='overflow-hidden'>
                    <h3 className='font-bold text-slate-900 text-base sm:text-lg truncate group-hover:text-indigo-600 transition-colors'>{session.role}</h3>
            ...
            <div className='flex items-center justify-between md:justify-end gap-6 w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0'>
                ...
                <div className='flex flex-col items-end gap-1.5'>
                    ...
                    <span className='text-indigo-600 font-bold text-xs flex items-center group-hover:text-violet-600 transition-colors'>{session.status === 'completed' ? 'Results' : 'Resume'}
