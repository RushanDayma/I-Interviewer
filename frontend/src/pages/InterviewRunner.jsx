...
function InterviewRunner() {
  ...
  const handleNavigation = (index) => {
    // Safely check length to prevent crashes
  ...
  const handleSubmitAnswer = async () => {
    ...
    // Validation checks if the entity is an actual, valid instance of a browser Blob file format
    if (audio && audio instanceof Blob) {
      formData.append('audioFile', audio, 'answer.webm'); // Assured it uses 'audioFile' for multer matching
    } else if (audio) {
      console.warn("Audio was detected but was not a valid file instance. Submitting code-only content.");
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm shadow-indigo-900/5 border border-slate-100 mb-6">
        <div>
          ...
          <div className="flex gap-2 mt-2">
            {activeSession?.question?.map((q, i) => (
              <div
                ...
                className={`w-3 h-3 rounded-full cursor-pointer transition-all ${i === currentQuestionIndex ? 'bg-indigo-600 scale-125 ring-2 ring-indigo-200' :
        ...
        <button
          ...
          disabled={isLoading}
          className="bg-rose-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-rose-700 disabled:opacity-50 transition-colors"
        >
          {isLoading ? "Finalizing..." : "Finish Interview"}
        </button>
      </div>

      {/* ✅ Fixed background color so the text is visible and pops nicely */}
      <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl shadow-indigo-900/10 mb-6 border border-slate-800">
        <span className="text-indigo-400 text-xs font-bold uppercase tracking-widest">Question {currentQuestionIndex + 1}</span>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center justify-center min-h-[300px]">
          ...
          {!isRecording && !currentDraft.audioBlob ? (
            <button
              ...
              className="w-20 h-20 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-full flex items-center justify-center text-white shadow-xl shadow-indigo-200 hover:scale-105 transition-all disabled:opacity-50 disabled:from-slate-400 disabled:to-slate-400 disabled:cursor-not-allowed"
            >
              🎤
            </button>
          ) : isRecording ? (
            <div className="text-center">
              <div className="w-20 h-20 bg-rose-500 rounded-full flex items-center justify-center animate-pulse text-white text-3xl cursor-pointer shadow-xl shadow-rose-200" onClick={stopRecording}>
                ⏹
              </div>
            ...
            <div className="text-center">
              ...
              {!isQuestionLocked && (
                <button onClick={() => setDrafts(prev => ({ ...prev, [currentQuestionIndex]: { ...prev[currentQuestionIndex], audioBlob: null } }))} className="text-xs text-slate-400 underline hover:text-rose-500 transition-colors">
                  Delete & Re-record
                </button>
              )}
            </div>
        ...
        <div className="bg-white p-2 rounded-3xl border border-slate-100 shadow-sm overflow-hidden h-[400px]">
          <div className="flex justify-between px-4 py-2 bg-slate-50 border-b border-slate-100">
            ...
            <select
              ...
              disabled={isQuestionLocked}
              className="text-xs bg-white border border-slate-200 rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-100 disabled:text-slate-400 transition-all"
            >
      ...
      {currentQuestion?.isEvaluated && currentQuestion?.aiFeedback && currentQuestion?.aiFeedback !== "Not evaluated yet" && (        <div className="mt-6 bg-emerald-50 border border-emerald-100 p-6 rounded-2xl animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="mt-4 flex gap-4">
            <span className="bg-white px-3 py-1 rounded-lg text-xs font-bold text-emerald-600 shadow-sm border border-emerald-100">Score: {currentQuestion.technicalScore}/100</span>
          </div>
        </div>
      )}

      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 p-4 px-6 md:px-12 flex justify-between items-center z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button
          onClick={() => handleNavigation(currentQuestionIndex - 1)}
          disabled={currentQuestionIndex === 0}
          className="text-slate-500 font-bold text-sm hover:text-indigo-600 disabled:opacity-30 transition-colors"
        >
          ← Previous
        </button>
        ...
        <div className="flex flex-col items-center">
          ...
          {isProcessing && message && (
            <div className="mb-2 text-xs font-mono text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full animate-pulse border border-indigo-100">
              🤖 {message}...
            </div>
          )}
          ...
          <button
            ...
            className={`px-8 py-3 rounded-xl font-bold text-white transition-all ${
              isProcessing ? 'bg-slate-400 cursor-wait shadow-none' :
              currentQuestion?.isEvaluated ? 'bg-emerald-500 shadow-emerald-200 shadow-lg' :
              isQuestionLocked ? 'bg-slate-400 shadow-none' :
              'bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 shadow-lg shadow-indigo-200 active:scale-95'
            }`}
          >
        ...
        <button
          onClick={() => handleNavigation(currentQuestionIndex + 1)}
          disabled={currentQuestionIndex === (activeSession?.question?.length || 0) - 1}
          className="text-slate-500 font-bold text-sm hover:text-indigo-600 disabled:opacity-30 transition-colors"
        >
          Next →
        </button>
