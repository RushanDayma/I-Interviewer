...
function SessionReview() {
    ...
    if (!activeSession || activeSession.status !== 'completed') {
        return (
            <div className="max-w-xl mx-auto mt-10 sm:mt-20 p-6 sm:p-10 bg-white rounded-3xl sm:rounded-[2.5rem] shadow-2xl text-center border border-slate-100 ">
                ...
                <Link to="/" className="inline-block bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-8 py-3 sm:px-10 sm:py-4 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-indigo-200 transition-all hover:from-indigo-700 hover:to-violet-700 active:scale-95 text-xs sm:text-sm">Dashboard</Link>
            </div>
        );
    }
    ...
    const barData = {
        ...
        datasets: [{
            ...
            backgroundColor: questions.map(q => (q.technicalScore || 0) > 70 ? '#4f46e5' : '#8b5cf6'),
            borderRadius: 8,
        }],
    };
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8 sm:space-y-12 animate-in fade-in duration-700">
            ...
            {/* --- Header --- */}
            {/* CHANGED: border-slate-100 to border-slate-700/50 for the dark background */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-700/50 pb-6 sm:pb-10">
                <div>
                    <span className="text-indigo-500 font-black uppercase tracking-[0.2em] text-[10px]">Assessment Complete</span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 overflow-x-auto pb-4 sm:pb-0 no-scrollbar snap-x">
                {[
                    { label: 'Overall Result', value: `${overallScore || 0}%`, color: 'indigo' },
                    // CHANGED: Added robust fallbacks for the undefined% bug
                    { label: 'Avg Technical', value: `${finalMetrics?.avgTechnical ?? finalMetrics?.avgTechnicalScore ?? 0}%`, color: 'slate' },
                    { label: 'Avg Confidence', value: `${finalMetrics?.avgConfidence ?? finalMetrics?.avgConfidenceScore ?? 0}%`, color: 'slate' },
                    { label: 'Session Time', value: formatDuration(startTime, endTime), color: 'slate' }
                ].map((stat, i) => (
                    <div key={i} className={`min-w-[160px] snap-center bg-white p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] shadow-sm border-l-[8px] ${stat.color === 'indigo' ? 'border-indigo-500' : 'border-slate-100'}`}>
                        <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">{stat.label}</p>
                        <p className={`text-2xl sm:text-4xl font-black mt-2 leading-none ${stat.color === 'indigo' ? 'text-indigo-600' : 'text-slate-800'}`}>{stat.value}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-6 sm:space-y-10">
                ...
                <h3 className="text-xl sm:text-3xl font-black text-white px-2 flex items-center tracking-tighter uppercase">
                    <span className="w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-indigo-500 to-violet-500 text-white rounded-xl sm:rounded-2xl flex items-center justify-center mr-3 sm:mr-5 text-base sm:text-xl shadow-md shadow-indigo-200/20">✓</span>
                    Answer Intelligence
                </h3>
                <div className="space-y-6 sm:space-y-10">
                    {questions.map((q, index) => (
                        <div key={index} className="bg-white rounded-3xl sm:rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden group hover:shadow-lg hover:shadow-indigo-900/5 transition-all duration-500">
                            <div className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                                ...
                                <div className="flex flex-col lg:flex-row justify-between items-start gap-4 sm:gap-6">
                                    <h4 className="text-lg sm:text-2xl font-bold text-slate-800 flex-1 leading-snug">
                                        <span className="text-indigo-500 mr-2 font-black italic">Q{index + 1}.</span> {sanitizeQuestionText(q.questionText)}
                                    </h4>
                                    <div className="flex gap-2 shrink-0">
                                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl sm:rounded-2xl border flex items-center gap-2 bg-emerald-50 border-emerald-100">
                                            ...
                                            <span className="text-xs sm:text-sm font-black text-emerald-600">{q.technicalScore || 0}%</span>
                                        </div>
                                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl sm:rounded-2xl border border-blue-50 bg-blue-50/30 flex items-center gap-2">
                                            ...
                                            <span className="text-xs sm:text-sm font-black text-blue-600">{q.confidenceScore || 0}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 pt-6 sm:pt-8 border-t border-slate-50">
                                    <div className="space-y-3">
                                        ...
                                        <div className="bg-slate-50/50 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] text-xs sm:text-sm italic text-slate-600 border-l-[4px] sm:border-l-[6px] border-indigo-500 leading-relaxed">
                                            "{q.aiFeedback}"
                                        </div>
                                    </div>
                                    <div className="space-y-3">
