...
export const sessionSlice = createSlice({
    ...
    reducers: {
        ...
        socketUpdateSession: (state, action) => {
            const { sessionId, status, message, sessionData, session } = action.payload;
            const payloadSession = sessionData || session;
            ...
            if (payloadSession && state.activeSession && state.activeSession._id === sessionId) {
                const currentQuestions = state.activeSession.question || state.activeSession.questions || [];
                const incomingQuestions = payloadSession.question || payloadSession.questions || [];

                // ✅ FIX 2: Map using the verified array references
                const updatedQuestions = currentQuestions.map((currentQ, index) => {
                    const incomingQ = incomingQuestions[index];
                ...
                state.activeSession.question = updatedQuestions;
                state.activeSession.questions = updatedQuestions;
                state.activeSession.overallScore = payloadSession.overallScore;
                state.activeSession.status = payloadSession.status;
                state.activeSession.matrix = payloadSession.matrix || payloadSession.metrics;
                state.activeSession.metrics = payloadSession.matrix || payloadSession.metrics;
            }
        },
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getSessionById.fulfilled, (state, action) => {
                state.activeSession = action.payload;
                console.log("New session data:", action.payload);
                state.isLoading = false;
            })
            ...
            .addCase(submitAnswer.fulfilled, (state, action) => {
                ...
                if (action.payload && Array.isArray(action.payload.question)) {
                    state.activeSession = action.payload;
                }
